...
# Default output directory for saving generated connectome renderings
outDir = os.path.join(os.path.abspath(os.path.expanduser(f"/cs/home/{getpass.getuser()}")), "con-imgs")
...
def compute_connectome(
    ...
    subidx : int
        Subject number
    outfolder : str or None
        Path to a directory for storing generated connectome renderings
        (must already exist). By default, (``outfolder = None``) renderings
        are stored in `/cs/home/$USER/con-imgs`
