...
def plot_real(
    ...
    ax.set_xlabel("$x$")
    ax.set_ylabel("$f(x)$")
    ax.set_title(f"The function {fstr} on $\mathbb{{R}}$")
    plt.show()
    return fig
...
def plot_complex(
    ...
    ax.set_title(f"The function {fzstr} on $\mathbb{{C}}$")
    plt.show()
    return fig
...
def plot_fractal1d(
    ...
    ax.set_xlabel("$x$")
    ax.set_ylabel("$f(x)$")
    ax.set_title(f"Convergence with {fstr} on $\mathbb{{R}}$")
...
def plot_fractal2d(
    ...
    ax.set_ylabel("$\Im(z)$")
    fzstr = fstr.replace("x", "z")
    ax.set_title(f"Convergence with {fzstr} on $\mathbb{{C}}$")
    norm = plt.Normalize(fmin, fmax)
    cbar = fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), norm=norm, ax=ax, ticks=[fmin, (fmin + fmax)/2.0, fmax])
    cbar.ax.set_yticklabels(["bad", "okay", "good"])
    plt.show()
    return fig
